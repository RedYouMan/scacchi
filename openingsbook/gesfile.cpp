...
bool setPieceIntoSquare(string com)
{
    ...
    while (mosse.size() > 0)
    {
        mossa = mosse.back();
        mosse.pop_back();
        // La obbligatorietà non viene richiesta per F: ,C:, L:,E: e Z: usate per la sospensione e, quindi, automatiche
        if (color == 'L' || color == 'C' || color == 'E' || color == 'Z' || color == 'U')
        {
...
void init()
{
    ...
    while (cont < 8)
    {
        if (cont == 1)
        {
            com = "T:Predispongo la scacchiera!\n";
        }
        if (cont == 2)
        {
...
void Sospendi(bool cambioColore)
{
    ...
    if (file_output.is_open())
    {
        ...
        if (cambioColore)
        {
            file_output << "T:cambio colore in corso\n";
        }
        else
        {
            file_output << "T: Ripresa partita precedente" << endl;
        }
        string com = "V:";
...
void viewValid()
{
    ...
    // in a8 e h8 dovrei trovare le torri  del Nero
